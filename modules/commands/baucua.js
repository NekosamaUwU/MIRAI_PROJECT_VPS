        ...
        const slotItems = ["ga", "tom", "bau", "cua", "ca", "nai"];
            const moneyUser = (await Currencies.getData(event.senderID)).money;
                var moneyBet = parseInt(args[1]);
                    ...
                    if (isNaN(moneyBet) || moneyBet <= 0) return api.sendMessage("[𝐁𝐎𝐓 🦋] => Mày bị mù à??? Số tiền đặt cược đâu đồ ngu???", event.threadID, event.messageID);
                if (moneyBet > moneyUser) return api.sendMessage("[𝐁𝐎𝐓 🦋] => Phèn như mày cũng chơi cái này à ? Cút về làm đỉ kiếm sống đi nghèo nàn rách rưới haha :)))", event.threadID, event.messageID);
            if (moneyBet < 100) return api.sendMessage("[𝐁𝐎𝐓 🦋] => Đéo có tiền thì cút? Ở trong cờ bạc đéo có khái niệm đặt bé :D", event.threadID, event.messageID);
