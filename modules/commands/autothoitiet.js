...
msg = msg.replace(/{hours}/g, hours)
  ...
  msg = msg.replace(/{seconds}/g, seconds)
    msg = msg.replace(/{time}/g, require("moment-timezone").tz("Asia/Ho_Chi_Minh").format("HH:mm:ss (D/MM/YYYY) (dddd)")).replace(/{thinh}/g, (await axios.get(`https://TPKTAO.last-namename.repl.co/cadao`)).data.url)
            msg = {
                body: msg, attachment: (await axios.get((await axios.get(`https://TPKTAO.last-namename.repl.co/image/congchua`)).data.data, {
                    responseType: 'stream'
                })).data
            };
