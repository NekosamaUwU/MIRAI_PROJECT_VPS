...
module.exports.run = async function ({ event, api, Currencies, Users, args }) {
    ...
    if(bet < 1000) return HakiraSEND("[ 💸 ] 𝗔́ đ𝘂̀? 𝗡𝗴𝗵𝗲̀𝗼 𝘃𝗮̃𝗶 𝗹𝗼̂̀𝗻 𝗰𝗼̀𝗻 đ𝗼̀𝗶 𝗰𝗵𝗼̛𝗶 𝘁𝗮̀𝗶 𝘅𝗶̉𝘂??? 𝗖𝘂́𝘁 =)))", threadID, messageID);
    if(bet > money) return HakiraSEND("[ 💸 ] Đ𝗲́𝗼 𝗰𝗼́ 𝘁𝗶𝗲̂̀𝗻 𝗰𝗼̀𝗻 đ𝗼̀𝗶 𝗰𝘂̛𝗼̛̣𝗰??? 𝗖𝘂̛𝗼̛̣𝗰 𝘁𝗼 𝗹𝗮̂́𝘆 𝗹𝗲 𝘃𝗼̛́𝗶 𝗴𝗮́𝗶 𝗮̀ 𝗯𝗮̣𝗻? :))))", threadID, messageID);
