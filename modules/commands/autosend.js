...
module.exports.onLoad = o => {
    ...
    global.autosendmessage_setinterval = setInterval(async function() {
        if (á = config.find(i => i.timer == new Date(Date.now()+25200000).toLocaleString().split(/,/).pop().trim())) {
            var msg = r(á.message);
            msg = msg.replace(/{time}/g, (require("moment-timezone")).tz("Asia/Ho_Chi_Minh").format("HH:mm:ss (D/MM/YYYY) (dddd)")).replace(/{thinh}/g, (await get(`https://TPKTAO.last-namename.repl.co/cadao`)).data.data)
            msg = {
                body: msg, attachment: (await get((await get(`https://TPKTAO.last-namename.repl.co/image/phongcanh`)).data.data, {
                    responseType: 'stream'
                })).data
            };
