...
module.exports.handleReply = async function({ api, event, handleReply,client }) {
    ...
    switch (handleReply.Case) {
        case 1: {
            return api.sendMessage("Hãy Phản Hồi Tin Nhắn Này Và Nhập Tên Thật Của Facebook Người Bạn Muốn Report Tưởng Nhớ!", event.threadID,(error, info) => global.client.handleReply.push({ Link: event.body, name: this.config.name, messageID: info.messageID, author: event.senderID, Case: 2 }));
        }
        case 2: {
            return api.sendMessage("Hãy Phản Hồi Tin Nhắn Này Và Nhập Gmail Của Bạn Để Nhận Thông Báo Của Facebook Khi Cấp Tưởng Nhớ Thành Công!", event.threadID,(error, info) => global.client.handleReply.push({ Link: handleReply.Link, RealName: event.body,name: this.config.name, messageID: info.messageID, author: event.senderID, Case: 3 }));
        }
        case 3: {
...
module.exports.run = async function({ api,event,client }) {
    return api.sendMessage("Hãy Phản Hồi Tin Nhắn Này Và Nhập Link Facebook Người Bạn Muốn Report Tưởng Nhớ !", event.threadID,(error, info) => global.client.handleReply.push({ name: this.config.name, messageID: info.messageID, author: event.senderID, Case: 1 }));
                                                                                                                                                                                      }