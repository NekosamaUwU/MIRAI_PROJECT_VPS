...
module.exports.config = {
  ...
  module.exports.run = async function({ api, event, args, Currencies, getText, permssion }) {
    try {
      ...
      if (isNaN(args[1]) == true) return api.sendMessage('[𝐒𝐍🌸] => 𝑼̉𝒂 𝒄𝒂́𝒊 𝒄𝒐𝒏 𝒄𝒂̣̆𝒄 𝒈𝒊̀ 𝒗𝒂̣̂𝒚 𝒃𝒂̣𝒏? 𝑻𝒊𝒆̂̀𝒏 đ𝒂̣̆𝒕 𝒍𝒂̀ 𝒄𝒐𝒏 𝒔𝒐̂́ 𝒎𝒂̀ 𝒐́𝒄 𝒍𝒐̛̣𝒏 :)?', threadID, messageID);
      var moneyBet = parseInt(args[1]);
      if (isNaN(moneyBet) || moneyBet <= 50) return api.sendMessage('[𝐒𝐍🌸] => 𝐂𝐨𝐧 𝐜𝐚̣̆𝐜!!! 𝐂𝐮́𝐭, đ𝐞́𝐨 𝐜𝐨́ 𝐭𝐢𝐞̂̀𝐧 𝐭𝐡𝐢̀ 𝐥𝐮̛𝐨̛̣𝐧 𝐜𝐡𝐨 𝐧𝐠𝐮̛𝐨̛̀𝐢 𝐤𝐡𝐚́𝐜 𝐯𝐚̀𝐨 𝐜𝐡𝐨̛𝐢 :))', threadID, messageID);
      if (moneyBet > money) return api.sendMessage('[𝐒𝐍🌸] => 𝐀́ đ𝐮̣ 𝐦𝐞̣ 𝐛𝐚̣𝐧 𝐲𝐞̂𝐮 𝐨̛𝐢? 𝐓𝐢𝐞̂̀𝐧 𝐭𝐡𝐢̀ đ𝐞́𝐨 𝐜𝐨́ 𝐦𝐚̀ 𝐜𝐨̀𝐧 𝐜𝐮̛𝐨̛̣𝐜 𝐭𝐨? 𝐋𝐚̂́𝐲 𝐥𝐞 𝐯𝐨̛́𝐢 𝐠𝐚́𝐢 𝐚̀ :))? 𝐂𝐮́𝐭 =))))', threadID, messageID);
      ...
      // sendMessage
      api.sendMessage({
        body: '🚧 Siu nhân bú lồn ra đây nè UwU \n Chờ đi :<<',
