...
module.exports.config = {
  ...
  module.exports.run = async function({ api, event, args, Currencies, getText, permssion }) {
    try {
      ...
      if (isNaN(args[1]) == true) return api.sendMessage('??? Mày đặt cược cái lồn gì vậy?', threadID, messageID);
      var moneyBet = parseInt(args[1]);
      if (isNaN(moneyBet) || moneyBet <= 50) return api.sendMessage('Tiền đéo có còn vào đây cua gái ? Cút đê :v', threadID, messageID);
      if (moneyBet > money) return api.sendMessage('Cút cút cút :D Đéo có tiền đừng có sĩ diện cược to lấy le với gái :)))', threadID, messageID);
