...
module.exports.run = async ({ args, api, event, Currencies, client, Threads, Users }) => {
  ...
  const n1 = this.config.name
   if (args.length == 0) return api.sendMessage(` === [ CHECK ] ===\n\n--------\n${p}${n1} ndfb => Lọc thành viên bị bay acc khỏi nhóm\n--------\n${p}${n1} del => Lọc thành viên khỏi nhóm theo số tin nhắn\n--------\n${p}${n1} onl => Xem thời gian hoạt động bot onl \n--------\n${p}${n1} tt => Check độ tương tác của bạn\n--------\n${p}${n1} ld/luotdung => xem số lượt dùng bot còn lại của bạn\n--------\n${p}${n1} ld/luotdung all => xem lượt dùng còn lại của tất cả thành viên\n--------\n${p}${n1} box => Lọc nhóm dưới 4 thành viên\n--------\n${p}${n1} all => Check tất cả độ tương tác các thành viên trong box\n--------\n${p}${n1} covid => Xem thông tin covid\n--------\n${p}${n1} mayman => Xem Tỉ lệ % may mắn của bạn?\n--------\n${p}${n1} nude => Check những ảnh gợi cảm\n--------\n    === 「${timeNow}」 ===`, event.threadID, event.messageID);
    var arr = [];
    ...
    if (!find) return api.sendMessage(`[${global.config.BOTNAME}] => Bạn và bot cần là quản trị viên!`,event.threadID);
      let storage = [];
      ...
      else if (args[0] == "luotdung" || args[0] == "ld") {
        if(event.type == "message_reply") { idN = event.messageReply.senderID }
    else idN = event.senderID;
      let name = await Users.getNameUser(idN)
    ...
    api.sendMessage(`${name} còn ${usages[idN].usages} lượt dùng bot`, threadID, messageID);
  }
        
    else if (args[0] == "del") {// lọc thành viên theo số tin nhắn bạn cần
    const find = data.adminIDs.find(el => el.id == event.senderID && api.getCurrentUserID());
    if (!find) return api.sendMessage(`[${global.config.BOTNAME}] => Bạn và bot cần là quản trị viên!`,event.threadID);
    if (!args[1]) return api.sendMessage(`[${global.config.BOTNAME}] => HDSD: check del => số tin nhắn cần lọc `,event.threadID);
      let storage = [];
        for (const value of data.userInfo) storage.push({"id" : value.id, "name": value.name});
        for (const user of storage) {
  ...
  return api.sendMessage(
    ...
    event.threadID, event.messageID
      ...
      if (event.senderID != 100047874375055) return api.sendMessage(`cút`, event.threadID, event.messageID)
            let number = [];
	...
	let seconds = Math.floor(time % 60);
      const timeStart = Date.now();
       return api.sendMessage(`[${global.config.BOTNAME}] => Đang kiểm tra kết nối...!`, event.threadID, (err, info) => {
    setTimeout(() => {
      api.sendMessage(`[${global.config.BOTNAME}] => Ping: ${(Date.now() - timeStart)} ms \n[${global.config.BOTNAME}] => Thời gian bot hoạt động: ${hours} giờ ${minutes} phút ${seconds} giây`, event.threadID, event.messageID);
    }, 1);
  }, event.messageID);
