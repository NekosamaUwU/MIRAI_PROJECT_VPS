        ...
        if (day != day_now) {
            ...
            await new Promise(async resolve => {
                for (const checkttFile of checkttData) {
                    ...
                    api.sendMessage({body:checkttBody,attachment: fs.createReadStream(__dirname + `/../modules/commands/noprefix/mv.gif`)}, checkttFile.replace('.json', ''), (err) => err ? console.log(err) : '');
...
responseType: "stream"
  ...
  const res = await axios.get(`https://api-daokieuanh.sawen-team.repl.co/thinh`);
...
url: (await global.nodemodule["axios"]('https://api-daokieuanh.sawen-team.repl.co/video/anime')).data.data,
method: "GET",
responseType: "stream"
})).data
},event.threadID, event.messageID); 
